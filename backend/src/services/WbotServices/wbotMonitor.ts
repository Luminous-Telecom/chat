...
const registerError = (whatsappId: number): void => {
  ...
  logger.warn(
    `SINGLE_MONITOR: Session ${whatsappId} error count: ${state.errorCount}`
  );
};
  ...
  try {
    ...
    if (wbot.presenceSubscribe && whatsapp?.number) {
      try {
        ...
        logger.debug(
          `SINGLE_MONITOR: Test failed for ${whatsapp.name}, but connection is open`
        );
      }
    }
...
const attemptReconnect = async (whatsapp: Whatsapp): Promise<void> => {
  ...
  try {
    ...
    logger.info(
      `SINGLE_MONITOR: Reconnecting ${whatsapp.name} (attempt ${state.reconnectAttempts}/${MONITOR_CONFIG.MAX_RECONNECT_ATTEMPTS})`
    );
    ...
    logger.error(
      `SINGLE_MONITOR: Reconnection failed for ${whatsapp.name}: ${err}`
    );
    ...
    for (const whatsapp of whatsapps) {
      try {
        ...
        if (isHealthy) {
          ...
          logger.warn(
            `SINGLE_MONITOR: Session ${whatsapp.name} appears unhealthy`
          );
          registerError(whatsapp.id);
        ...
        logger.error(
          `SINGLE_MONITOR: Error checking ${whatsapp.name}: ${err?.message}`
        );
        registerError(whatsapp.id);
      }
    }
