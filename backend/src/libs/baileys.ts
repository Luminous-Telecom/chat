...
const wsUndefinedChecks = new Map<
  number,
  {
    count: number;
    lastCheck: number;
...
const shouldCheckWsUndefined = (whatsappId: number): boolean => {
  ...
  if (check.count >= WS_UNDEFINED_CONFIG.MAX_CHECKS) {
    baseLogger.warn(
      `Session ${whatsappId} ws undefined checks exceeded limit, ignoring`
    );
    return false;
  }
...
export const getBaileysSession = (
  ...
  if (!isUsable) {
    baseLogger.warn(
      `Session ${whatsappId} is not usable - connection: ${connectionState}, ws: ${wsState}`
    );
  }
...
async function handleWsUndefined(
  ...
  const now = Date.now();
  if (
    check.count >= WS_UNDEFINED_CONFIG.MAX_CHECKS &&
  ...
  try {
    baseLogger.warn(
      `Handling ws undefined state for ${whatsapp.name} (attempt ${check.count}/${WS_UNDEFINED_CONFIG.MAX_CHECKS})`
    );
    ...
    baseLogger.error(
      `Failed to handle ws undefined state for ${whatsapp.name}: ${err}`
    );
  } finally {
...
export async function initBaileys(
  ...
  try {
    ...
    if (phoneNumber) {
      baseLogger.info(
        "Modo de número detectado - limpando sessão completamente"
      );
      ...
      if (existingSession) {
        baseLogger.info(
          `Removendo sessão existente antes de iniciar modo número`
        );
    ...
    if ((wbot as any).ws) {
      const { ws } = wbot as any;

      ws.on("error", async (err: Error) => {
        baseLogger.error(
          `WebSocket error for ${whatsapp.name}: ${err.message}`
        );

        // Se estiver em modo número, tentar reconectar
        if (phoneNumber) {
          baseLogger.info(
            `Tentando reconectar WebSocket para ${whatsapp.name}`
          );
          try {
      ...
      ws.on("close", async (code: number, reason: string) => {
        baseLogger.info(
          `WebSocket closed for ${whatsapp.name} (code: ${code}, reason: ${reason})`
        );

        // Se estiver em modo número e o código for 1006, tentar reconectar
        if (phoneNumber && code === 1006) {
          baseLogger.info(
            `Tentando reconectar após fechamento inesperado do WebSocket`
          );
          try {
            await safeReconnect(wbot, whatsapp);
          } catch (reconnectErr) {
            baseLogger.error(
              `Erro ao reconectar após fechamento: ${reconnectErr}`
            );
          }
        }
      });
    ...
    wbot.ev.on("connection.update", async update => {
      ...
      if (connection === "open" && !(wbot as any).ws) {
        baseLogger.warn(
          `Session ${whatsapp.name} has open connection but ws is undefined`
        );
      ...
      if (lastDisconnect) {
        ...
        if (shouldReconnect) {
          baseLogger.info(
            `Session ${whatsapp.name} disconnected with status ${statusCode}, attempting to reconnect...`
          );
          ...
          baseLogger.info(
            `Session ${whatsapp.name} logged out, not attempting to reconnect`
          );
          await whatsapp.update({
            status: "DISCONNECTED",
            qrcode: "",
      ...
      if (connection === "close") {
        ...
        if (!shouldReconnect) {
          if (statusCode === DisconnectReason.loggedOut || statusCode === 401) {
            baseLogger.info(
              `Session ${whatsapp.name} logged out or auth failed - clearing session and requiring new QR`
            );
            ...
            baseLogger.error(
              `Fatal error for ${whatsapp.name} (${statusCode}), not reconnecting`
            );
            await whatsapp.update({
              status: "DISCONNECTED",
              qrcode: "",
        ...
        if (temporaryErrors.includes(statusCode)) {
          ...
          baseLogger.info(
            `Temporary error (${statusCode}) - short retry delay`
          );
        } else {
        ...
        const timeout = setTimeout(async () => {
          try {
            ...
            baseLogger.info(
              `Reconnection attempt ${newRetries} for ${whatsapp.name}`
            );
            ...
            baseLogger.error(
              `Reconnection attempt ${newRetries} failed: ${err}`
            );

            if (newRetries >= MAX_RECONNECT_ATTEMPTS) {
              baseLogger.error(
                `All reconnection attempts exhausted for ${whatsapp.name}`
              );
              await whatsapp.update({
                status: "DISCONNECTED",
                qrcode: "",
    ...
    if (phoneNumber) {
      ...
      const waitForConnection = async () => {
        ...
        while (attempts < maxAttempts) {
          try {
            baseLogger.debug(
              `Tentativa ${attempts + 1} de aguardar conexão para pairing...`
            );
            ...
            baseLogger.debug(
              `Conexão pronta para pairing na tentativa ${attempts + 1}`
            );
            return;
          } catch (err) {
            attempts++;
            baseLogger.warn(
              `Falha na tentativa ${attempts} de aguardar conexão: ${err.message}`
            );

            if (attempts < maxAttempts) {
              baseLogger.info(
                `Tentando reconectar antes da próxima tentativa...`
              );
              try {
                await safeReconnect(wbot, whatsapp);
        ...
        throw new Error(
          `Falha em todas as ${maxAttempts} tentativas de estabelecer conexão`
        );
      };
      ...
      try {
        ...
        if (typeof wbot.requestPairingCode === "function") {
          try {
            baseLogger.debug(
              `Iniciando requestPairingCode para ${whatsapp.name}`
            );
            baseLogger.debug(
              `Tentando gerar pairing code para número: ${phoneNumber}`
            );
            ...
            baseLogger.debug(
              `Pairing code enviado para o frontend via WebSocket`
            );
          } catch (err) {
            baseLogger.error(
              `Erro detalhado ao gerar pairing code para ${whatsapp.name}:`,
              {
                error: err,
            ...
            if (err.message.includes("auth") || err.message.includes("401")) {
              baseLogger.error(
                `Erro de autenticação durante pairing: ${err.message}`
              );
            }
          }
        } else {
          baseLogger.error(
            "requestPairingCode não está disponível nesta versão do Baileys.",
            {
        ...
        baseLogger.error(
          `Erro ao aguardar conexão para pairing: ${err.message}`
        );
        throw err;
      }
    }
